...
const encryptionCred = CryptoJS.enc.Utf8.parse(process.env.NEXT_PUBLIC_KEY);

export const decryptPnr = (encrypted, path) => {
  try {
    var decryptedString = AES.decrypt(encrypted, encryptionCred, {
      iv: encryptionCred,
    });
...
export const encryptPnr = (string) => {
  var encrypted = AES.encrypt(string, encryptionCred, {
    iv: encryptionCred,
  });
