  ...
  (
    ...
    const mapClass = (
      ...
      switch (SeatGroup) {
        ...
        case (SeatGroup = 2):
          ...
          break;
        case (SeatGroup = 3):
          const code3 = legend?.data?.items.filter(
            (seat) => seat.seat_group === SeatGroup
          )[0].color_code;
          seatCode = `!bg-[${code3}]`;
          break;
        ...
        case (SeatGroup = 5):
          ...
          break;
        case (SeatGroup = 6):
          const code = legend?.data?.items.filter(
            (seat) => seat.seat_group === SeatGroup
          )[0].color_code;
          seatCode = `!bg-[${code}]`;
          break;
    ...
    const clickContent = (seat) => {
      return (
        <div className="text-primary-main text-base p-2">
          <p className="mb-2 font-light">
            <span className="font-semibold">{seat.SeatDesignator}</span>
            {" - "}
            {mapSeatGroup(seat.SeatGroup).name}
          </p>
                  ...
                  {!isLoading &&
                    legend?.data.items.map((legend, index) => (
                      <div className="seats__legend__item" key={index}>
                        <figure>
                          <div
                            className={`eat-box w-full h-full rounded`}
                            style={{ backgroundColor: `${legend.color_code}` }}
                          ></div>
                        </figure>
                        <p>{legend.name}</p>
                      </div>
