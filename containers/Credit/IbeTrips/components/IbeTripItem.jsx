...
const IbeTripItem = ({ journey, schedueIndex }) => {
  ...
  const { creditGoTrip, creditReturnTrip } = useSelector(creditSelector);
  ...
  return (
    <section className="flex flex-col mb-6">
      <section className="ibe__trip__item relative">
        ...
        {creditGoTrip?.journey?.JourneySellKey.toLowerCase() ===
        journey?.JourneySellKey.toLowerCase() ? (
          <p className="bg-primary-main text-green py-1 px-2  rounded-[4px] absolute top-[12px] text-[8px]">
            Go Selected -{" "}
            {creditGoTrip?.fare?.RuleNumber.toLowerCase() === "savr" && "gSaver"}
            {creditGoTrip?.fare?.RuleNumber.toLowerCase() === "flex" && "gFlex"}
            {creditGoTrip?.fare?.RuleNumber.toLowerCase() === "clsc" && "gClassic"}
          </p>
        ) : null}

        {creditReturnTrip?.journey?.JourneySellKey.toLowerCase() ===
        journey?.JourneySellKey.toLowerCase() ? (
          <p className="bg-primary-main text-green py-1 px-2  rounded-[4px] absolute top-[12px] text-[8px]">
            Return Selected -{" "}
            {creditReturnTrip?.fare?.RuleNumber.toLowerCase() === "savr" && "gSaver"}
            {creditReturnTrip?.fare?.RuleNumber.toLowerCase() === "flex" && "gFlex"}
            {creditReturnTrip?.fare?.RuleNumber.toLowerCase() === "clsc" &&
              "gClassic"}
          </p>
        ) : null}
      </section>
